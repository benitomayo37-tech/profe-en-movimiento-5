...
foreach ($requiredPath in @($welcomePath, $aiHeroPath, $photoPath)) {
...
export default function WelcomeSection({
  ...
  return (
    <section className="relative rounded-3xl bg-gradient-to-br from-blue-950 via-blue-800 to-blue-600 px-6 py-8 text-white shadow-xl sm:px-8 lg:px-10">
      <div className="relative z-10 max-w-4xl md:pr-[270px] lg:pr-[330px]">
      ...
      {/* Fotografia exclusiva del Dashboard, recortada a medio cuerpo. */}
      <div
        aria-hidden="true"
        className="pointer-events-none absolute -bottom-10 -top-10 right-3 z-20 hidden w-[280px] overflow-hidden md:block lg:right-6 lg:w-[340px]"
      >
        <Image
          ...
          height={1536}
          priority
          className="absolute right-[-18px] top-0 h-auto w-[330px] max-w-none drop-shadow-[0_16px_20px_rgba(0,0,0,0.28)] lg:right-[-10px] lg:w-[380px]"
        />
      </div>
    </section>
  );
...
if ($welcomeCheck -notmatch '/images/profe-armando-hero\.png') {
    throw "La fotografia no pudo instalarse en WelcomeSection.tsx"
}

if ($welcomeCheck -notmatch 'recortada a medio cuerpo') {
    throw "No se pudo confirmar el nuevo Hero del Dashboard"
}
...
Write-Host "- Dashboard: fotografia instalada en el componente visible y recortada a medio cuerpo." -ForegroundColor Green
