...
export default async function AgentsPage({ searchParams }: { searchParams: Promise<{ conversation?: string }> }) {
  ...
  const conversationLimit = access.hasProAccess ? 30 : 3;
  const { data: conversationsData } = await supabase.from("ai_agent_conversations").select("id,title,last_specialist,created_at,updated_at").eq("user_id", access.userId).order("updated_at", { ascending: false }).limit(conversationLimit);
  ...
  const { data: newestMessages } = conversationId ? await supabase.from("ai_agent_messages").select("id,conversation_id,role,content,specialist,response_kind,saved_at,created_at").eq("conversation_id", conversationId).eq("user_id", access.userId).order("created_at", { ascending: false }).limit(100) : { data: [] };
  const messagesData = [...(newestMessages ?? [])].reverse();
  ...
  const { data: microcycleUsage } = !access.hasProAccess
    ? await supabase.from("monthly_agent_feature_usage").select("run_count").eq("user_id", access.userId).eq("usage_month", month.toISOString().slice(0, 10)).eq("feature_key", "microcycle").maybeSingle()
    : { data: null };
  ...
  const microcycleRemaining = access.hasProAccess ? null : Math.max(0, 1 - (microcycleUsage?.run_count ?? 0));
  ...
  return <AppLayout sidebar={<Sidebar />} header={<div className="flex min-h-20 items-center justify-between gap-4 px-6"><div><h1 className="text-lg font-bold text-slate-950">Centro de Agentes IA</h1><p className="text-sm text-slate-500">Coordinador docente y especialistas</p></div><AccountBadge authenticated email={access.email} fullName={access.fullName} /></div>} footer={<div className="px-6 py-4 text-center text-xs text-slate-500">Profe en Movimiento 5.0 · El docente conserva la decisión final</div>}>
    <Container size="wide" className="space-y-7 py-8"><section className="rounded-[2rem] bg-gradient-to-br from-slate-950 via-blue-950 to-violet-900 p-8 text-white shadow-xl sm:p-10"><div className="flex flex-wrap items-center gap-3"><p className="text-xs font-black uppercase tracking-[.2em] text-orange-300">Coordinación inteligente</p><span className={`rounded-full px-3 py-1 text-xs font-black uppercase tracking-wide ${access.hasProAccess ? "agent-plan-pro-badge" : "bg-white/15 text-white"}`}>Plan {access.hasProAccess ? "Pro" : "Free"}</span>{access.role === "admin" ? <span className="agent-admin-badge rounded-full px-3 py-1 text-xs font-black uppercase tracking-wide">Administrador</span> : null}</div><h1 className="mt-3 text-4xl font-black">Un equipo de agentes para tu trabajo docente y deportivo</h1><p className="mt-4 max-w-3xl leading-7 text-blue-100">Planificación, Evaluación, Inclusión y Entrenamiento Deportivo colaboran bajo un Coordinador. Cada resultado queda en tu conversación y solo se guarda cuando tú lo confirmas.</p></section><AgentsWorkspace initialConversations={conversations} initialMessages={messagesData as AgentMessage[]} initialConversationId={conversationId} initialRemaining={remaining} monthlyLimit={monthlyLimit} initialMicrocycleRemaining={microcycleRemaining} hasProAccess={access.hasProAccess} /></Container>
  </AppLayout>;
}
