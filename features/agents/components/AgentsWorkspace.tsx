...
function AgentMessageContent({ content }: { content: string }) {
  ...
  for (let index = 0; index < lines.length;) {
    ...
    if (line.includes("|") && index + 1 < lines.length && isTableSeparator(lines[index + 1])) {
      ...
      blocks.push(<div key={`table-${index}`} className="my-4 overflow-x-auto rounded-xl border border-slate-300"><table className="min-w-[760px] w-full border-collapse text-left text-xs leading-5"><thead className="bg-blue-950 text-white"><tr>{headers.map((header, cellIndex) => <th key={cellIndex} className="border border-blue-800 px-3 py-2 font-black">{header}</th>)}</tr></thead><tbody>{rows.map((row, rowIndex) => <tr key={rowIndex} className="odd:bg-white even:bg-slate-50">{headers.map((_, cellIndex) => <td key={cellIndex} className="border border-slate-300 px-3 py-2 align-top text-slate-700">{row[cellIndex] ?? ""}</td>)}</tr>)}</tbody></table></div>);
      continue;
    }
