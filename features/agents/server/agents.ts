...
Considera los materiales indicados como un inventario cerrado. No añadas, supongas ni recomiendes conos, aros, marcas, fichas, petos, silbatos, cronómetros, pizarras u otros objetos que el usuario no haya declarado disponibles. Puedes utilizar líneas existentes del espacio, el propio cuerpo, palmas, voz, gestos o agrupaciones sin presentarlos como materiales adicionales.
...
Distribuye exactamente los materiales declarados y no incorpores apoyos impresos, señales físicas ni objetos adicionales si no fueron proporcionados. Sustituye esos recursos por demostración, instrucciones verbales, gestos, compañeros de apoyo y referencias ya existentes en el espacio.`,
});
...
Para cada sesión utiliza obligatoriamente esta tabla Markdown compacta:
| Tiempo | Actividad | Organización y desarrollo | Criterio observable |
|---|---|---|---|
Cada fila representa una actividad completa. Tiempo debe expresarse en minutos enteros, por ejemplo "12 min"; no mezcles dentro de la celda series con pausas cuya suma resulte ambigua. Incluye recuperación, hidratación y transición dentro del tiempo de la fila. Añade después de cada tabla una línea "Comprobación temporal: ... = [duración exacta] min" con la suma aritmética de todas sus filas. Antes de responder verifica que esa suma coincida exactamente con la duración solicitada.
...
export async function runTeacherCoordinator(input: string) {
  ...
  const responseKind = rawOutput.startsWith("[[ACLARACION]]") ? "clarification" as const : "result" as const;
  const output = rawOutput.replace(/^\[\[(?:RESULTADO|ACLARACION)\]\]\s*/i, "").trim();
  if (!output) throw new Error("empty_agent_output");
  return { output, specialist: "coordinator" satisfies AgentSpecialist, responseKind };
}
