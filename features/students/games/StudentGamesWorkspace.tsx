...
function Puzzle({ topic }: { topic: GameTopic }) {
  ...
  return <div className="mx-auto max-w-xl text-center"><h4 className="text-2xl font-black text-slate-950">Ordena las piezas del 1 al 8</h4><p className="mt-2 text-slate-600">Movimientos: <strong>{moves}</strong></p><div className="mx-auto mt-6 grid max-w-md grid-cols-3 gap-3 rounded-[2rem] bg-violet-950 p-4 shadow-xl">{tiles.map((tile, index) => <button key={tile || "empty"} type="button" onClick={() => move(index)} className={`aspect-square rounded-2xl text-3xl font-black shadow-inner transition ${tile ? "bg-gradient-to-br from-violet-200 to-fuchsia-300 text-violet-950 hover:scale-95" : "bg-violet-900"}`}>{tile ? <><span>{icon}</span><span className="ml-1 text-base">{tile}</span></> : ""}</button>)}</div>{solved && <p className="mt-6 rounded-2xl bg-emerald-100 p-5 text-xl font-black text-emerald-900">¡Puzzle completado en {moves} movimientos! 🎉</p>}</div>;
}
