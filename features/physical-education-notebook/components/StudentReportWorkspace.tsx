...
export default function StudentReportWorkspace({
  ...
  return (
    <section id="grade-summary-print" className="space-y-6 rounded-3xl border-2 border-blue-200 bg-slate-950/30 p-6">
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-sm font-bold text-slate-300">{course.name} - {course.schoolYear}</p>
      ...
      <div className="space-y-4">
        {periods.map((period) => {
          ...
          return (
            <article key={period.id} className="rounded-2xl border border-slate-600 bg-slate-900/60 p-4">
              <div className="flex flex-wrap items-center justify-between gap-2">
                <h2 className="text-xl font-black text-white">{period.name}</h2>
                <span className="rounded-full bg-slate-700 px-3 py-1 text-xs font-black uppercase text-slate-200">{period.status}</span>
              </div>
...
function Metric({ label, value, emphasis = false }: { label: string; value: string; emphasis?: boolean }) {
  return (
    <div className="rounded-xl border border-slate-700 bg-slate-950/60 p-3">
      ...
      <p className={emphasis ? "mt-1 text-2xl font-black text-blue-300" : "mt-1 text-xl font-black text-white"}>{value}</p>
    </div>
  );
}