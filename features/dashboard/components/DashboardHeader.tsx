  ...
  return (
    <div className="flex min-h-20 items-center gap-4 px-6">
        ...
        <div className="hidden items-center gap-2 sm:flex">
          <span className={`rounded-full px-3 py-1.5 text-xs font-black uppercase tracking-wide ${access?.hasProAccess ? "bg-violet-100 text-violet-700" : "bg-slate-100 text-slate-600"}`}>
            Plan {access?.hasProAccess ? "Pro" : "Free"}
          </span>
          {access?.role === "admin" ? <span className="rounded-full bg-amber-100 px-3 py-1.5 text-xs font-black uppercase tracking-wide text-amber-800">Administrador</span> : null}
        </div>
